...
for car_key, car_value in car_data.items():
    if car_value[1] >= search_criteria[0] and car_value[2] >= search_criteria[1] and car_value[4] <= search_criteria[2]:
        filtered_cars.append((car_key, car_value))

sorted_cars = sorted(filtered_cars, key=lambda car: car[1][4])

top_5_cars = sorted_cars[:5]
result = dict(top_5_cars)
print(result)